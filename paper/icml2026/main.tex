...
% Allow more floats per page to prevent figure* deferral
...
% --- Architecture Fig 1: placed here so it floats to top of next page (method section) ---
\begin{figure*}[t]
\centering
...
\caption{Sensor and satellite encoders. AquaSSM (left) places eight continuous-time SSM channels in parallel from $1$\,h to $365$\,d, ingesting irregularly sampled multivariate streams. HydroViT (right) MAE-pretrains a ViT-S/16 on $2{,}986$ Sentinel-2 tiles, then fine-tunes on $4{,}202$ satellite--in-situ pairs for 9 water quality parameters.}
\label{fig:enc1}
\end{figure*}
...
% --- Architecture Fig 2: placed here (after fusion text) so it floats to next available page ---
\begin{figure*}[t]
\centering
\begin{subfigure}[t]{0.30\textwidth}\includegraphics[width=\linewidth]{fig_microbiomenet.jpg}\caption{MicroBiomeNet (8.4M params).}\label{fig:microbe}\end{subfigure}\hfill
\begin{subfigure}[t]{0.34\textwidth}\includegraphics[width=\linewidth]{fig_toxigene.png}\caption{ToxiGene ($145.2$M params).}\label{fig:toxigene}\end{subfigure}\hfill
\begin{subfigure}[t]{0.34\textwidth}\includegraphics[width=\linewidth]{fig_biomotion.jpg}\caption{BioMotion (3M params).}\label{fig:biomotion}\end{subfigure}
\caption{Microbial, molecular, and behavioral encoders. MicroBiomeNet (a) performs CLR projection from the Aitchison simplex into Euclidean space and integrates a neural ODE for temporal dynamics. ToxiGene (b) routes RNA-seq through a Reactome-constrained gene$\rightarrow$pathway$\rightarrow$process$\rightarrow$outcome hierarchy with sparse biologically-informed connections. BioMotion (c) uses diffusion pretraining on healthy \emph{Daphnia} trajectories and detects toxicant exposure as denoising reconstruction error.}
\label{fig:enc2}
\end{figure*}
...
\begin{table}[t]
\caption{SENTINEL-DB composition by source. Records are quality-tiered (Q1--Q4) and spatially indexed with H3 resolution~8.}
\label{tab:db}
\centering
\small
\begin{tabular}{lrc}
...
Per-encoder benchmarks (Table~\ref{tab:encoders}; full visual comparison in Appendix Figure~\ref{fig:bench}) place each branch ahead of or competitive with the strongest published single-stream baseline on its own held-out test set: AquaSSM at AUROC $0.916$ ($+0.052$ over MCN-LSTM); HydroViT v9 at $R^2\!=\!0.893$ on water temperature ($+0.009$ over DenseNet121); BioMotion at AUROC $1.000$ at the assay level ($+0.042$ over a Deep Autoencoder); MicroBiomeNet at macro-F$_1\!=\!0.899$ ($-0.006$ vs.\ tuned SimpleMLP); and ToxiGene at $7$-class macro-F$_1\!=\!0.877$ as a first-in-class supervised method.

\begin{table}[t]
\caption{Per-encoder benchmarks with $95\%$ CIs. Metrics: AUROC for AquaSSM/BioMotion; $R^2$ for HydroViT; macro-F$_1$ for MicroBiomeNet/ToxiGene.}
\label{tab:encoders}
\centering
\small
...
\midrule
AquaSSM & $.916\;[.934,.943]$ & MCN-LSTM $.864$ & $+.052$ \\
HydroViT v9 & $.893$ & DenseNet $.884$ & $+.009$ \\
MicroBiomeNet & $.899\;[.903,.923]$ & SimpleMLP $.905$ & $-.006$ \\
...
Figure~\ref{fig:bench} provides a visual comparison of each SENTINEL encoder against published SOTA and tuned classical baselines.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig_encoder_benchmarks.jpg}
\caption{Per-encoder benchmarks vs.\ published SOTA and tuned classical baselines. SENTINEL encoders (green) match or exceed the strongest reported model on each modality's own held-out test set.}
\label{fig:bench}
\end{figure*}

\section{Ablation and Modality Importance}
\label{app:ablation}

Figure~\ref{fig:abl} provides a visual breakdown of leave-one-out modality criticality and cumulative fusion gain.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig_modality_importance.jpg}
\caption{Where the fusion gain comes from. \textbf{Left}: leave-one-out modality criticality on the windowed $31$-event protocol ($100$ random-drop trials). Sensor ($-0.246$) and behavioral ($-0.174$) are the most consequential. \textbf{Right}: cumulative AUROC gain from successive cascade tiers, from sensor-only ($0.943$) to full $5$-modal fusion ($0.992$, $p = 0.002$).}
\label{fig:abl}
\end{figure*}

\section{Operational Reliability and Causal Chains}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig_causal_chains.jpg}
