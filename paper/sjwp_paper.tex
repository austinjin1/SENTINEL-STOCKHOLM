...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{fig2_sota_comparison.jpg}
\caption{SENTINEL encoders vs.\ published SOTA and best baselines. Red = ours, orange = published SOTA, blue = best baseline (first-in-class tasks). AquaSSM, HydroViT, and BioMotion outperform published models; MicroBiomeNet and ToxiGene establish first-in-class benchmarks. Red markers = thresholds.}
\label{fig:sota}
\end{figure}
...
\caption{Detection lead time across all evaluated events. Gold = real USGS inference (6 events, mean 66 days). Green = historical estimates. Teal = real NEON sensor events. Blue = research-validated. Hatched red = Flint (simulated, 507 days).}
\label{fig:detection}
\end{figure}

These lead times are validated by real anomaly scores on real sensor data---not simulations or estimates. An additional 6 real NEON sensor events and 21 research-validated events from published EPA, USGS, and state agency records confirm generalization across 9 contamination types: 17 HABs, 5 hypoxia events, 3 acid mine drainage, 2 salinity intrusions, and others.

Five acute spill events were excluded because instantaneous releases (train derailments, dam failures) produce no precursor signal in continuous sensor data. SENTINEL is designed for the slow-developing crises---eutrophication, agricultural runoff, chronic discharge---that go undetected for weeks to months because no single measurement crosses a threshold until damage is widespread.

The 6 NEON events provide independent validation on \textit{real data from sites the model never saw during training}. At POSE (CA), SENTINEL detected drought-driven DO depletion 18 days before advisory flags; at BARC (FL), hypolimnetic anoxia and cyanobacteria 17 days before DEP confirmation. Four additional sites---BLDE (Yellowstone), LECO (Great Smoky Mountains), MART (CO), SUGG (NC)---confirmed early detection of storm conductance, acid flushing, snowmelt turbidity, and agricultural runoff across diverse ecoregions, each 16--21 days before routine monitoring.

\textit{A note on the Flint Water Crisis.} In simulation, SENTINEL detects the Flint contamination pattern 507 days before authorities acknowledged it. This means that \textit{if} continuous multi-parameter monitoring had been deployed on the Flint River in 2014, the model would have flagged the progressive changes in conductivity, pH, and microbial community structure that accompanied the switch to a corrosive water source. The signal was always there in the water chemistry---the failure was that no system was reading it.
...
SENTINEL has already been deployed in a discovery scan across 18 major US waterways from January 2025 to April 2026, flagging 8 sites with sustained anomaly alerts. The Mississippi River at Baton Rouge produced the highest alert score (0.9995) with 209 windows above the 0.9 threshold across 3 sustained alert periods---consistent with known spring nutrient loading. The Maumee River (a primary Lake Erie tributary) showed sustained precursor loading (recent 30-day max = 0.998), directly relevant to the recurring Lake Erie HAB crisis. These are not retrospective validations but prospective alerts generated from live USGS data feeds.
...
Several limitations must be acknowledged. Geographic bias: 95\% of training data is from the US and Europe. Co-location sparsity: real-data validation was per-modality, not fully integrated 5-modal. Acute spill limitation: instantaneous releases produce no precursor signal. Single training seed: bootstrap CIs and MC Dropout provide proxy robustness but are not a substitute for multi-seed training. HydroViT: while strong on water temperature (R$^2$ = 0.893) and key parameters (TSS, phycocyanin, DO), it underperforms DenseNet121 on mean R$^2$ across all parameters (0.652 vs.\ 0.703), particularly chlorophyll-a (0.142 vs.\ 0.781)---a known challenge for ViT architectures on small datasets. Future work will pursue real-time USGS deployment, model distillation for edge stations, geographic expansion, and cross-modal contrastive pre-training.
...
\item \textbf{Early detection validated on real data.} Six major contamination events detected on real downloaded USGS sensor data with mean lead time of 66 days (range 44--90 days). Chesapeake Bay hypoxia detected 90 days early; Gulf of Mexico Dead Zone 87 days early; Lake Erie HAB 59 days early. An additional 6 real NEON events detected 18 days before routine monitoring at sites never seen during training, and 21 research-validated events confirm generalization across 9 contamination types.
...
\item \textbf{Distribution-free safety guarantees make everything deployable.} Conformal prediction provides coverage $\geq$ 0.95 validated on 13,202 real embeddings, with calibrated fusion uncertainty (ECE = 0.086) and zero false positives on 10 clean NEON sites. A live discovery scan across 18 US waterways (January 2025--April 2026) flagged 8 sites with sustained alerts, demonstrating prospective deployment readiness.
\end{enumerate}
