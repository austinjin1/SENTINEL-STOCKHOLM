...
\usepackage{setspace}\setstretch{1.08}      % Near single spacing
...
\centering
\includegraphics[width=0.58\textwidth]{fig_encoder_aquassm.jpg}
\caption{AquaSSM sensor encoder. Eight parallel CT-SSM channels (1h--365d) process irregularly-sampled multi-parameter sensor data. Trained on 291,855 USGS sequences (AUROC = 0.939).}
\end{subfigure}
\vspace{2pt}
\begin{subfigure}[t]{\textwidth}
\centering
\includegraphics[width=0.58\textwidth]{fig_encoder_hydrovit.jpg}
\caption{HydroViT v9 satellite encoder. CNN-ViT hybrid with MAE pretraining on 2,986 Sentinel-2 patches and fine-tuning on 5,464 paired samples (water temp R$^2$ = 0.893).}
\end{subfigure}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.30\textheight]{fig_encoder_microbiomenet_v2.jpg}
\caption{MicroBiomeNet microbial encoder (8.4M parameters). 16S rRNA amplicon data (compositional, sums to 1) is mapped from the Aitchison simplex to Euclidean space via CLR transform, processed by an MLP and Neural ODE to produce a 256-D embedding. Trained on 20,288 EMP samples (F1 = 0.913).}
\label{fig:enc_micro}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{fig_encoder_toxigene_v2.jpg}
\caption{ToxiGene molecular encoder (145.2M parameters). \textbf{Left}: Hierarchical sparse architecture mirrors the biological gene $\rightarrow$ pathway $\rightarrow$ process $\rightarrow$ adverse outcome hierarchy, with sparse connections constrained by Reactome and AOP-Wiki. Trained on 84K genes $\times$ 4 transcriptomic datasets plus 268K EPA ECOTOX chemical records. \textbf{Right}: Biologically-informed sparsity---network connectivity mirrors known gene $\rightarrow$ pathway $\rightarrow$ process $\rightarrow$ outcome wiring, with 145.2M parameters constrained to mechanistically plausible relationships. Projections show the multi-level AOP hierarchy and sparsity callouts.}
\label{fig:enc_toxi}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.58\textwidth]{fig_encoder_biomotion.jpg}
\caption{BioMotion behavioral encoder. Denoising diffusion U-Net learns healthy \textit{Daphnia magna} trajectory distributions; toxicant-exposed trajectories produce high reconstruction error. Trained on 28,610 EPA ECOTOX tests (AUROC = 1.000, +0.042 vs Deep AE [25]).}
\label{fig:biomotion}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig5_ablation.jpg}
\caption{Modality ablation. \textbf{Left}: AUROC drop when each modality is removed---sensor ($-0.246$) and behavioral ($-0.174$) are most critical. \textbf{Right}: Cumulative build-up from sensor-only (0.943) to full fusion (0.992, $p = 0.002$).}
\label{fig:ablation}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig4_case_studies.jpg}
\caption{Detection lead time for 31 contamination events. Green = real USGS inference (6, 44--90 days); teal = real NEON (6); blue = research-validated (21). Mean lead: 32 days.}
\label{fig:detection}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_lake_erie_timeline.jpg}
\caption{Lake Erie HAB 2023 detection timeline. Satellite detects chlorophyll-a at Day $-10$; sensors activate at Day $-5$; fusion crosses 0.80 at Day $-3$; causal chain (TP $\rightarrow$ COD) identifies eutrophication. Lead time: +201.6 hours.}
\label{fig:timeline}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{fig_causal_network_v2.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig9_fpr_persistence.jpg}
\caption{False positive and alert persistence. \textbf{Left}: Zero false alerts on 10 clean NEON sites (specificity = 1.0) vs.\ 10--100\% detection on contamination events. \textbf{Right}: Persistence ratio of 31:0---contamination events sustain up to 89 consecutive above-threshold windows while clean sites produce zero.}
\label{fig:fpr}
\end{figure}
