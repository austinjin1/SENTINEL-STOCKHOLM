...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{fig_encoder_biomotion.jpg}
\caption{BioMotion behavioral encoder. A denoising diffusion U-Net learns the distribution of healthy \textit{Daphnia magna} locomotion trajectories by reconstructing clean data from progressively corrupted inputs. At inference, toxicant-exposed trajectories produce high reconstruction error as the anomaly score---no threshold tuning required. Trained on 28,610 real EPA ECOTOX concentration-response tests spanning 1,391 chemicals. AUROC = 1.000, outperforming the published Deep Autoencoder (PLOS CompBio 2024) by +0.042 AUROC on 10$\times$ more data.}
\label{fig:biomotion}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig5_ablation.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig9_fpr_persistence.jpg}
...
% Annex removed to fit page limit. Conformal theory: P(z_{n+1} in C_alpha) >= 1-alpha.
% All models trained on a single NVIDIA RTX 4060 (8 GB); total 223.1M params, ~72h.
